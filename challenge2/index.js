        ...
        if(originState<0 || originState>52){
            ...
            if(destinyState<0 || destinyState>52|| originState==destinyState){
                ...
                while(flag){
                    ...
                    packageAmount = parseInt(prompt("How many packages do you wish to send ? The max is 500 and the min is 100\n"));
                    ...
                    if(packageAmount<100 || packageAmount>500){
                        ...
                        if(packageAmount>200){
                            packageOption = parseInt(prompt("The amount of packages is greater than 200 so the price per kilometer would be $60 and it's recommended to use a larger truck\n\nDo you whis to continue?\n1-Yes\n2-No\n\n"));
                            switch(packageOption){
                                case 1:
                                    //I asume that the truck is going at 100km per hour
                                    alert(`the price would be $${(distance*60).toFixed(2)} and it would take ${(distance/100).toFixed(2)} hours`);
                                    flag = false;
                                    break;
                                case 2:
                                    break;
                            ...
                            alert(`the price would be $${(distance*50).toFixed(2)} and it would take ${(distance/100).toFixed()} hours`);
                            flag = false;
                        }
                    }
                }
